...
@WebServlet("/AccountantFeeReport")
public class AccountantFeeReport extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		for (StudentBean bean : list) {
			out.println(
					...
					+ "<td>" + bean.getPaid() + "</td>"
					+ "<td>" + bean.getDue()+ "</td>"
					+"<td><a href='EditStudentForm?RollNo="+bean.getId()+"'>Edit Student</a></td>"
					+"<td><a href='DeleteStudent?RollNo="+bean.getId()+"'>Delete Student</a></td>"
					+ "</tr>");
		}
		out.println("</table>"
				+ "<a href='AccountantHome.html'><=back</a>"
				+ "</body>"
				+ "</html>");
		}
		else {
